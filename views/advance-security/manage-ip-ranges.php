		...
		<div class="col-md-12">
						...
						<div class="form-body">
								...
								<div class="col-md-6">
									<div class="form-group">
										<label class="control-label">
											<?php echo esc_attr( $lab_manage_ip_ranges_start ); ?> :
											<span class="required" aria-required="true">*</span>
										</label>
										<input type="text" class="form-control" name="ux_txt_start_ip_range" id="ux_txt_start_ip_range"  onblur="check_valid_ip_ranges_limit_attempts_booster(this);" onfocus="prevent_paste_limit_attempts_booster(this.id);" value="" onkeyPress="limit_attempts_booster_valid_ip_address(event);" placeholder="<?php echo esc_attr( $lab_manage_ip_ranges_start_placeholder ); ?>">
										<i class="controls-description"><?php echo esc_attr( $lab_manage_ip_ranges_start_tooltip ); ?></i>
									</div>
								</div>
								<div class="col-md-6">
									<div class="form-group">
										<label class="control-label">
											<?php echo esc_attr( $lab_manage_ip_ranges_end ); ?> :
												<span class="required" aria-required="true">*</span>
											</label>
											<input type="text" class="form-control" name="ux_txt_end_ip_range" id="ux_txt_end_ip_range"  onblur="check_valid_ip_ranges_limit_attempts_booster(this);" onfocus="prevent_paste_limit_attempts_booster(this.id);"  value="" onkeyPress="limit_attempts_booster_valid_ip_address(event);" placeholder="<?php echo esc_attr( $lab_manage_ip_ranges_end_placeholder ); ?>">
											<i class="controls-description"><?php echo esc_attr( $lab_manage_ip_ranges_start_tooltip ); ?></i>
									</div>
								</div>
							</div>
							<div class="form-group">
								...
								<select name="ux_ddl_range_blocked" id="ux_ddl_range_blocked" class="form-control">
									...
									<option value="24hours"><?php echo esc_attr( $lab_twenty_four_hours ); ?></option>esc_attr
							...
							<div class="form-group">
								...
								<textarea class="form-control" name="ux_txtarea_manage_ip_range" id="ux_txtarea_manage_ip_range" rows="4" placeholder="<?php echo esc_attr( $lab_comments_placeholder ); ?>"></textarea>
								<i class="controls-description"><?php echo esc_attr( $lab_manage_ip_ranges_comments_tooltip ); ?></i>
							</div>
							<div class="line-separator"></div>
							<div class="form-actions">
								<div class="pull-right">
									<input type="button" class="btn vivid-green" name="ux_btn_clear" id="ux_btn_clear" value="<?php echo esc_attr( $lab_clear ); ?>" onclick="value_blank_limit_attempts_booster();"/>
									<input type="submit" class="btn vivid-green" name="ux_btn_advance_security_ip_range_submit" id="ux_btn_advance_security_ip_range_submit" value="<?php echo esc_attr( $lab_manage_ip_ranges_block ); ?>">
								</div>
							</div>
						</div>
					</form>
			...
			<div class="portlet box vivid-green">
				<div class="portlet-title">
					<div class="caption">
						<i class="icon-custom-wrench"></i>
						<?php echo esc_attr( $lab_manage_ip_ranges_view_blocked ); ?>
					</div>
				</div>
				<div class="portlet-body form">
					<form id="ux_view_manage_ip_ranges">
								...
								<div class="col-md-6">
									<div class="form-group">
										...
										<div class="input-icon-custom right">
											<input type="text" class="form-control" value="<?php echo esc_attr( date( 'm/d/Y', $start_date ) ); ?>" name="ux_txt_start_date" onkeypress="prevent_data_limit_attempts_booster(event)" id="ux_txt_start_date"  placeholder="<?php echo esc_attr( $lab_start_date_placeholder ); ?>">
						...
						<div class="form-actions">
							<div class="pull-right">
								<input type="submit" class="btn vivid-green" name="ux_btn_ip_range" id="ux_btn_ip_range" value="<?php echo esc_attr( $lab_submit ); ?>">
							</div>
						</div>
							<div class="line-separator"></div>
							...
							<table class="table table-striped table-bordered table-hover table-margin-top" id="ux_tbl_manage_ip_range">
								<thead>
									<tr>
										...
										<th>
										<label class="control-label">
											<?php echo esc_attr( $lab_ip_ranges ); ?>
										</label>
									</th>
									<th>
										<label class="control-label">
									...
									foreach ( $manage_ip_range as $row ) {
										...
										<tr>
											<td style="text-align: center;width: 5%;">
												<input type="checkbox" onclick="check_all_limit_attempts_booster('#ux_chk_all_manage_ip_range');" name="ux_chk_manage_ip_range_<?php echo esc_attr( $row['meta_id'] ); ?>" id="ux_chk_manage_ip_range_<?php echo esc_attr( $row['meta_id'] ); ?>" value="<?php echo esc_attr( $row['meta_id'] ); ?>">
											</td>
											<td>
												<label>
													<?php $ip_address = explode( ',', $row['ip_range'] ); ?>
													<?php echo esc_attr( long2ip_limit_attempts_booster( $ip_address[0] ) ); ?> - <?php echo esc_attr( long2ip_limit_attempts_booster( $ip_address[1] ) ); ?>
												</label>
											</td>
										<td
											<label>
										...
										<td class="custom-alternative" style="width: 10%;">
											<a href="javascript:void(0);" class="btn limit-attempts-booster-buttons" onclick="delete_ip_range_limit_attempts_booster(<?php echo intval( $row['meta_id'] ); ?>)"><?php echo esc_attr( $lab_delete ); ?></a>
										</td>
									</tr>
									<?php
									}
