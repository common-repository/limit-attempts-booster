<?php
/**
 * Template for system information settings.
 *
 * @author  tech-banker
 * @package limit-attempts-booster/views/system-information
 * @version 2.0.0
 */
...
if ( ! is_user_logged_in() ) {
	...
	if ( ! $access_granted ) {
		return;
	} elseif ( SYSTEM_INFORMATION_LIMIT_ATTEMPTS_BOOSTER === '1' ) {
		?>
		<div class="page-bar">
						...
						<div class="custom-form-body">
							...
							<table class="table table-striped table-bordered table-hover" >
								...
								<tr>
									...
									<td>
										<span>
											<?php echo esc_attr( bloginfo( 'version' ) ); ?>
										</span>
									</td>
								</tr>
								<tr>
									...
									$params         = array(
										'sslverify'  => false,
										'timeout'    => 60,
										'user-agent' => 'limit_attempts_booster',
										'body'       => $request,
									);
									...
									<td>
										<span>
											<?php echo esc_html( isset( $_SERVER['SERVER_SOFTWARE'] ) ? wp_unslash( $_SERVER['SERVER_SOFTWARE'] ) : '' );  // WPCS: input var ok, sanitization ok. ?>
										</span>
									</td>
									</tr>
									<tr>
									...
									global $wpdb, $lab;
									// Get MYSQL Version.
									$sql_version = $wpdb->get_var( 'SELECT VERSION() AS version' );// WPCS: db call ok, cache ok.
									// GET SQL Mode.
									$my_sql_info = $wpdb->get_results( 'SHOW VARIABLES LIKE "sql_mode"' );// WPCS: db call ok, cache ok.
