<?php
/**
 * Template for blocked ip range settings.
 *
 * @author  tech-banker
...
if ( ! is_user_logged_in() ) {
	...
	if ( ! $access_granted ) {
		...
		$delete_ip_ranges = wp_create_nonce( 'limit_attempts_delete_ip_ranges' );
		?>
		<div class="page-bar">
		...
		<div class="col-md-12">
			<div class="portlet box vivid-green">
				...
				<div class="portlet-body form">
					<form id="ux_frm_view_blocked_ip_Ranges">
						<div class="form-body">
						<div class="table-top-margin">
							<select name="ux_ddl_last_manage_ip_ranges" id="ux_ddl_last_manage_ip_ranges" class="custom-bulk-width">
						...
						<table class="table table-striped table-bordered table-hover table-margin-top" id="ux_tbl_manage_ip_ranges">
							<thead>
								<tr>
									<th style="text-align: center; width: 5%;" class="chk-action">
									<input type="checkbox" name="ux_chk_all_manage_ip_ranges" id="ux_chk_all_manage_ip_ranges">
								</th>
									<th>
							...
							<tbody>
								<?php
								foreach ( $manage_ip_ranges as $row ) {
									?>
									<tr>
									<td style="text-align: center; width: 5%;">
										<input type="checkbox" onclick="check_all_limit_attempts_booster('#ux_chk_all_manage_ip_ranges');" name="ux_chk_manage_ip_ranges_<?php echo esc_attr( $row['meta_id'] ); ?>" id="ux_chk_details_<?php echo esc_attr( $row['meta_id'] ); ?>" value="<?php echo esc_attr( $row['meta_id'] ); ?>">
									</td>
									<td>
										<label>
									...
									<td>
										...
										<td class="custom-alternative" style="width: 10%;">
											<a href="javascript:void(0);" class="btn limit-attempts-booster-buttons" onclick="delete_ip_ranges_last_logins(<?php echo intval( $row['meta_id'] ); ?>)"><?php echo esc_attr( $lab_delete ); ?></a>
										</td>
									</tr>
									<?php
								}
